...
def get_logics(app, delta: timedelta):
    date = get_date(delta)
...
async def index(request: Request):
    ...
    cache_logic, logic = get_logics(redelta=delta)
...
async def yesterday_top(request: Request):
    logic, cache_logic = get_logics(app=request.app, delta=request.app.state.days_delta + timedelta(days=1))
    cache = await cache_logic.cache
    yesterday_sims = await logic.get_similarities(cache)
    return templates.TemplateResponse(
        'closest1000.html',
        context=dict(yesterday=sorted(yesterday_sims.items(), key=lambda ws: ws[1], reverse=1)))
...
async def secrets(request: Request, api_key: Optional[str] = None):
    ...
    return templates.TemplateResponse(
        'all_secrets.html',
        context=dict(secrets=sorted(secrets, key=lambda ws: ws[1], reverse=1)),
    )
...
async def faq(request: Request):
    ...
    return templates.TemplateResponse(
        'faq.html',
        context=dict(yesterday=cache[-11:])
    )
